...
新打包布局的根目录只保留一个启动程序 `MaaBanG.exe`，界面和运行依赖位于 `app/`。配置、日志及任务报告分别在 `app/config/`、`app/logs/`、`app/debug/`；开发环境的报告路径仍为项目根目录的 `debug/`。更新时请完整解压新包，不要单独复制 exe。
...
- **目标数量是每位角色最终希望达到的数量，不是本次购买件数**。范围 0–999；默认 0 不解锁，只返回主页。
...
所有任务正常完成后返回游戏主页，包含无可领取奖励、资源不足、次数用尽和没有待处理项目的情况。贴纸交换的“贴纸交换分类”默认全选，可取消不需要的分类；全部取消时不交换，只返回主页。贴纸交换会实际消耗米歇尔贴纸；每日免费招募不使用星石或招募券。不会购买月度付费奖励或自动建立邀请关系。确认结果不明时停止，避免重复提交。

日常报告及错误截图位于 `debug/daily/`，启动和验证说明见[日常任务开发文档](docs/zh_cn/develop/daily_tasks.md)。

## 清火自动演出

**仅从游戏内“收藏→所有”选歌：请先在游戏里收藏目标歌曲。界面的歌曲目录不代表你的收藏，未收藏的歌曲无法找到。**

添加“清火自动演出”任务，配置完成后从主页启动。

- 模式：自由演出，或巡回演出中的自由巡演。
- 歌曲：使用 Bestdori 中国服目录；只在收藏中查找，找不到即停止。同名歌曲按乐队区分。
- 难度候选随歌曲联动，只显示目录中中国服已开放的难度。旧配置中的 Special 不可用时使用 Expert；报告记录实际难度。
- 每首消耗 0–3 火；火不足可选择停止，或降低到剩余火数（最低 0）。停止策略下，巡演开始前要求火数足够完成三首。
- 最大演出次数可留空，表示运行到自动额度不足或火不足策略要求停止。填入数字则限制完整轮数：自由演出一首计一轮，自由巡演三首计一轮。
- 每次开演前确认游戏内“自动演出 开”，不会进行手动打歌。每日自动演出上限为 10 首，巡演一轮需要 3 次，额度不足不开始新一轮。
- 演出中每 10 秒检查一次状态，结算后处理奖励、评级升级及演出后对话，正常停止后返回主页。不会补充火或购买自动次数。

报告和错误截图位于 `debug/auto_live/`。详细说明见[自动演出开发文档](docs/zh_cn/develop/auto_live.md)。

歌曲数据离线保存在 `agent/data/songs_cn.json`，可阅读的表格见[中国服歌曲目录](docs/data/songs_cn.csv)。更新目录及界面选项：`python tools/update_song_catalog.py`。来源、筛选规则及未实机验证范围见[歌曲目录说明](docs/zh_cn/develop/song_catalog.md)。
...
```powershell
python tools/install.py v0.3.1
```
