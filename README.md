...
| 功能 | 支持内容 |
| --- | --- |
| 清火自动演出 | 自由演出、自由巡演；从游戏收藏选歌，配置难度、火数和最大演出次数 |
| 默认 3D 演出服装 | 全部 8 队 40 人，或指定乐队、成员；按目标数量解锁，领奖后不换装 |
| 主页礼物 | 一键领取，直到礼物箱为空 |
...
> **仅从游戏内「收藏 → 所有」选歌。** 请先在游戏里收藏目标歌曲。软件中的中国服歌曲目录不代表你的收藏，未收藏的歌曲无法找到，任务会停止。
...
新增目录歌曲及同名歌曲版本尚未逐首实机验证，游戏文字或目录差异可能导致找歌失败。
...
```powershell
python tools/install.py v0.3.2-dev1
```
