...
完成后返回主页。贴纸交换的“贴纸交换分类”默认全选，可取消不需要的分类；全部取消时不操作游戏。贴纸交换会实际消耗米歇尔贴纸；每日免费招募不使用星石或招募券。不会购买月度付费奖励或自动建立邀请关系。确认结果不明时停止，避免重复提交。

日常报告及错误截图位于 `debug/daily/`，启动和验证说明见[日常任务开发文档](docs/zh_cn/develop/daily_tasks.md)。

## 清火自动演出

添加“清火自动演出”任务，先把目标歌曲加入游戏收藏，并从主页启动。

- 模式：自由演出，或巡回演出中的自由巡演。
- 歌曲：SAVIOR OF SONG、EXIST；只在收藏中查找，找不到即停止。
- 难度：Easy、Normal、Hard、Expert、Special。歌曲没有可用 Special 时使用 Expert，并写入报告。
...
```powershell
python tools/install.py v0.3.0
```
