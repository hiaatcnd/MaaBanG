...
UI 的“贴纸交换分类”支持多选成员、表情、服装、背景、其他，默认全选。全部取消时不进入交换商店、不消耗贴纸，返回游戏主页后结束。报告的 `categories` 记录本次选中的分类。命令行省略 `--categories` 时仍交换全部五类；仅交换表情和背景的示例：

```powershell
uv run --project agent python tools/run_daily_task.py ExchangeMichelle --categories 表情 背景 --adb "C:/Program Files/Netease/MuMu/nx_main/adb.exe"
```

## 流程与边界

- 礼物箱使用一键领取；任务列表使用各分类的全部领取，并识别禁用按钮。未建立邀请关系时跳过，不创建邀请码。
- 贴纸交换按成员、表情、服装、背景、其他的顺序遍历已勾选分类，跳过未勾选分类与已达上限的项目。可交换项目排在列表顶部；每次交换数量为 1，交换后重新识别列表，避免项目移位。顶部没有“交换”按钮时立即结束当前分类，不向下翻页。
