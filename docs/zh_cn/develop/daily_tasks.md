...
UI 的“贴纸交换分类”支持多选成员、表情、服装、背景、其他，默认全选。全部取消时直接结束，不导航或消耗贴纸。报告的 `categories` 记录本次选中的分类。命令行省略 `--categories` 时仍交换全部五类；仅交换表情和背景的示例：
...
- 贴纸交换按成员、表情、服装、背景、其他的顺序遍历已勾选分类，跳过未勾选分类与已达上限的项目。每次交换数量为 1，交换后重新识别列表，避免项目移位。连续两次滚动后的列表一致时结束该分类。
