...
3. 选择“解锁默认3D演出服装”，输入每位角色的目标数量 x（0–999，0 不执行）。
   “服装执行范围”默认为全部成员；选择“指定乐队”后选择一支乐队，或选择“指定成员”后选择一位成员。任务仅处理所选范围，x 对其中每位角色生效。
...
- **快速模式：若收集任务全部完成，直接跳过该角色，不统计服装列表。** 即使 x 较大，也不对这类角色继续补齐。这是用户选定的速度优先行为。
- 达到 x 的角色跳过。其余角色按列表顺序寻找未持有服装，确认“默认配色”、裁缝套装图标、套装与金币数量。
...
在仓库根目录用 PowerShell 执行，以下示例只检查 x=60：
...
测试原始记录保存在本机 `debug/probe-all-report.json`、`debug/test-one-before.png`、`debug/test-one-after.png`、`debug/test-one-resources-after.png` 及 `debug/grid-count-log.txt`。Agent IPC 的本机限制见上节。
